  ...
  return html`
    <div style="margin-top: 12px;">
      ...
      <label class="field">
        ...
        <input type="password" name="repo-token" placeholder="ghp_..." autocomplete="off" />
      </label>
      <button
        class="btn primary"
  ...
  return html`
    <div style="margin-top: 12px;">
      <label class="field">
        ...
        <input type="text" name="repo-org" placeholder="my-org" autocomplete="off" />
      </label>
      <label class="field" style="margin-top: 8px;">
        <span>Repo name</span>
        <input type="text" name="repo-name" placeholder="my-skills" autocomplete="off" />
      </label>
      <button
        class="btn primary"
...
function renderInitialized(
  ...
  return html`
    <div style="margin-top: 12px;">
      ...
      ${status.lastCommit ? html`<div class="muted" style="margin-top: 8px;">Last commit: ${status.lastCommit}</div>` : nothing}
      <div class="row" style="margin-top: 12px; gap: 8px; flex-wrap: wrap;">
        <button class="btn primary" ?disabled=${isBusy} @click=${() => props.onPush()}>
          ${busy === "push" ? "Pushing…" : "Push"}
        </button>
        <button class="btn" ?disabled=${isBusy} @click=${() => props.onPull()}>
          ${busy === "pull" ? "Pulling…" : "Pull"}
        </button>
        <button class="btn" ?disabled=${isBusy} @click=${() => {
        ...
        <button class="btn" ?disabled=${isBusy} @click=${props.onRefresh}>
